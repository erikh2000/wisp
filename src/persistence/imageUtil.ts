...
import {MIMETYPE_PNG} from "./mimeTypes";
...
export async function storeImageAtPath(path:string, imageFilename:string, pngImageBytes:Uint8Array):Promise<string> {
    if (imageFilename === UNSPECIFIED_IMAGE_KEY) throw new Error(`Cannot store image with filename of "${UNSPECIFIED_IMAGE_KEY}".`);
    
    let suffixCount = 0
    const [filename] = splitFilenameAndExtension(imageFilename);
    ...
    while(true) {
        ...
        if (!keysAtPath.includes(imageKey)) {
            await setBytes(imageKey, pngImageBytes, MIMETYPE_PNG);
            return imageKey;
        }
        ...
        if (!skipByteComparison) {
            ...
            if (existingImageBytes && _areBytesEqual(existingImageBytes, pngImageBytes)) return imageKey; // The same image is already stored.
        }
